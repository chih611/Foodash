...
const Confirm = () => {
  ...
  const [infos] = useState([
    ...
    {id: 4, type: "address", data: "Unit 5, 104 Lord St., Bundoora, Vic, 3083"}
  ]);
  ...
  const [items] = useState([
    {id: 1, qty: 2, desc: "Box(16) - Spiced Potato Samosa (vegan)", unitPrice: 50.00, total: 250.00},
    {id: 2, qty: 2, desc: "Box(16) - Seasonal Green and Ricotta Rolls", unitPrice: 50.00, total: 250.00},
    {id: 3, qty: 2, desc: "Box(12) - Carrot Cake", unitPrice: 50.00, total: 250.00},
    {id: 4, qty: 80, desc: "Mixed Drinks", unitPrice: 5.00, total: 400.00}
  ]);
      ...
      <div ref={contentRef}>
        ...
        <div className="text-center my-4"> 
          ...
          <p className="subtitle my-5">
            ...
            We will prepare and deliver to you shortly.
        ...
        <div className="text-center my-4">
          {orders.map((order) => (
            <p key={order.id} className="subtitle my-1"> {order.type}: {order.data} </p>
          ))}
        </div>

        <div className="d-flex justify-content-center mx-5 p-5" style={{borderBottom: "solid 1px #ecbf9c"}}>
          <div className="mx-2">
            <h4 className="my-2"> Deliver To: </h4>
            <ConfirmLayout>                
              {infos.map((info) => (
                <div key={info.id} className="w-100 d-flex justify-content-between">
        ...
        <div className="d-flex justify-content-around mx-5 my-5">
          <div>
            ...
            {items.map((item) => (
              <p key={item.id} className="subtitle">{item.qty} </p> ))}
          </div>
          <div>
            <h4 className="mb-3"> PRODUCT </h4>
            {items.map((item) => (
              <p key={item.id} className="subtitle">{item.desc} </p> ))}
          </div>
          <div>
            <h4 className="mb-3"> UNIT PRICE</h4>
            {items.map((item) => (
              <p key={item.id} className="subtitle">{item.unitPrice.toFixed(2)} </p> ))}
          </div>
          <div>
            <h4 className="mb-3"> TOTAL </h4>
            {items.map((item) => (
              <p key={item.id} className="subtitle">{item.total.toFixed(2)} </p> ))}
          </div>
        </div>

        <div className="text-end mx-5 p-5" style={{borderTop: "solid 1px #ecbf9c"}}>
          <h4 className="my-2">Total Cost</h4>
          <p className="subtitle">(including gst and fees)</p>
          <h5 className="my-2">${items.reduce((sum, item) => sum + item.total, 0).toFixed(2)}</h5>
        </div>
